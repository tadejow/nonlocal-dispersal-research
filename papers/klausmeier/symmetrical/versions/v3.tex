...
\end{lem}
\begin{proof}
    Observe that $\mathcal{A}$ is a bounded, linear operator. Thus, it generates a uniformly continuous semigroup $\{\mathcal{T}_u(t)\}_{t \geq 0}$. Moreover, spectrum of operator $\mathcal{L}$ is contained in real left semi-axis, see \cite[Thm. \ldots]{ChasseigneChavesRossi2006} and \cite[Theorem 2.1]{Tadej2025} for details. From that, the growth bound $\omega_0$, defined below, satisfies
    \begin{equation*}
        \omega_0 := \inf\{\omega \in \mathbb{R}: \lim_{t \rightarrow \infty} e^{-\omega t} \|\mathcal{T}_u(t)\| = 0 \} < 0.
    \end{equation*}
    Hence, $\mathcal{T}_u$ is a contraction semigroup, see \cite[Proposition V.1.7]{EngelNagel2000} for details.
\end{proof}
\begin{cor}
    It follows from Lemma \ref{lem:A_SEMIGROUP_GENERATOR}, that $\mathcal{A}$ is an analytic operator.
\end{cor}
...
\subsection{Existence of solutions} We reduce the system \eqref{eq:u_stationary}-\eqref{eq:v_bound_stationary} to the following boundary-value problem
\begin{numcases}{}
...
\begin{thm}
    ...
    \end{equation}
    with $R > 0$ being sufficiently small and some $\epsilon := \epsilon(d_v) > 0$.
\end{thm}
...
\begin{proof}
    ...
    \end{equation}
    Denote the Banach space $X = \left\{ \varphi \in C_{0,\Omega}(\mathbb{R}^n): \|\varphi\|_\infty \leq R \right\}$. Then, define operator $\mathcal{F}: X \rightarrow X$, corresponding to equation \eqref{eq:u-stationary-fixed-point}, by the formula
    \begin{align}
    ...
    \begin{align}
        ...
        \end{split}
    \end{align}
    Consider the function $g(\varphi) = (2u_*\varphi + \varphi^2)V(u_* + \varphi)$. Applying Lemmas \ref{lem:existence_of_inhomogeneous_elliptic_v} and \ref{lem:stationary_v_lipschitz_constant} we obtain the following bound
    \begin{align*}
        \|g'(\varphi)\|_\infty 
        &= 
    ...
    \begin{equation}
        \|U - u_*\|_{L^\infty(\Omega)} = \|\varphi\|_{L^\infty(\Omega)} = \| \leq R.
    \end{equation}
...
\subsection{Insufficient initial biomass} Here, we investigate how the assumption of small initial density leads to the propagation of desert.
...
\begin{thm}
    ...
    \end{equation}
    where we denoted the water profile $V_0 := V(0)$, obtained in Lemma \ref{lem:existence_of_inhomogeneous_elliptic_v}.
\end{thm}
\begin{proof}
...
\begin{rem}
    ...
    The stationary solution $(u_{*,1},v_{*,1})$ corresponds to the desert state, particularly $u_{*,1} = 0$. On the other hand, solutions $(u_{*,2},v_{*,2})$ and $(u_{*,3},v_{*,3})$ are ecologically more interesting as they correspond to the equilibria with a non-zero biomass $u_{*,i}$.
\end{rem}

\begin{thm}
    \label{thm:Stationary-Solutions-Stability}
    For all $A > 0$ and $B > 0$ there exists a regime of dispersal speed $d_u > 0$ and water diffusivity $d_v > 0$ such that each of the stationary solutions $(U, V)$ constructed in Theorem \ref{thm:Stationary-Solutions-Existence} is linearly, asymptotically stable in the sense of the norm of $L^2(\Omega)$.
\end{thm}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \end{pmatrix},
    \end{align*}
    From that, we write the energy equation as follows
    \begin{align}
    ...
    \begin{align}
        ...
        \end{split}
    \end{align}
    where we denoted the real numbers $C_1:=C_1(A,B)$ and $C_2:=C_2(A,B)$, related to the parameters $A, B$. Now, suppose that
    \begin{equation}
        \max\left\{\left(d_u \beta_1 + C_1
            \right), \left(d_v \lambda_1 + C_2\right)\right\} < 0.
    \end{equation}
    Then, when solving differential inequality \eqref{eq:energy_combined}, one obtains linear, asymptotic stability of $(U, V)$. In particular
    \begin{equation}
...
\subsection{Truncation of the kernel}
