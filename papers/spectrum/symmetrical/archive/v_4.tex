...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Less rigorous assumption
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

While Theorem 2.2 guarantees that $\beta_1$ cannot be lower than the continuous spectrum, it does not prevent $\beta_1$ from collapsing into $\sigma_c$, which would result in the non-existence of the first non-trivial eigenfunction. To ensure the existence of an isolated eigenvalue $\beta_1$, we must show that $\beta_1 > \sup \sigma_c$. 

We will prove that this strict spectral gap always exists for a broad class of physically relevant dispersal kernels, specifically those that are strictly decreasing with respect to distance.

\begin{lem}
    \label{lem:spectral_gap_decreasing}
    Let $\Omega \subset \mathbb{R}^n$ be an open, bounded domain. Assume the integral kernel $J \in C^0(\mathbb{R}^n)$ satisfies Assumption 1.1, and additionally, that $J$ is radially symmetric and strictly decreasing with respect to distance: $J(z) = \tilde{J}(|z|)$ where $\tilde{J}: [0, \infty) \to [0, \infty)$ is strictly decreasing. Then
    \begin{equation}
        \sup \sigma_c < \beta_1.
    \end{equation}
\end{lem}
...
\begin{proof}
Recall that the infimum of the continuous spectrum is given by $\sup \sigma_c = \max_{x \in \overline{\Omega}} (-b(x)) = -\min_{x \in \overline{\Omega}} b(x)$, where $b(x) = \int_\Omega J(x-y) \, dy$. Since $\overline{\Omega}$ is compact and $b(x)$ is continuous, the minimum is attained at some point $x_1 \in \overline{\Omega}$. 

To prove $\beta_1 > \sup \sigma_c$, it suffices to construct a single test function $v \in A_1$ such that $\langle \mathcal{L}v, v \rangle > \sup \sigma_c$. Since $J$ is radially symmetric and strictly decreasing, $J(0) > J(z)$ for any $z \neq 0$. By the continuity of $b(x)$, for any $\varepsilon > 0$, there exists a $\delta > 0$ such that for all $x \in \Omega$ satisfying $|x - x_1| < \delta$, we have $b(x) - b(x_1) < \varepsilon$. 

We fix a second point $x_2 \in \Omega$ distinct from $x_1$ such that $|x_1 - x_2| =: D < \delta$. We choose $\varepsilon$ sufficiently small to satisfy $\varepsilon < J(0) - J(D)$. Thus, we guarantee that:
\begin{equation} \label{eq:strict_gap_condition}
    b(x_2) - b(x_1) < J(0) - J(D).
\end{equation}

For any sufficiently small radius $R \in (0, D/2)$, let $B_1 = B(x_1, R) \cap \Omega$ and $B_2 = B(x_2, R) \cap \Omega$. By the choice of $R$, the sets are disjoint ($B_1 \cap B_2 = \emptyset$). We further assume $R$ is chosen such that $|B_1| = |B_2| =: \mu_R > 0$ (if $x_1 \in \partial \Omega$, we can select $x_1', x_2' \in \text{Int}(\Omega)$ arbitrarily close to $x_1, x_2$ to ensure equal volumes while preserving \eqref{eq:strict_gap_condition}).

We define the test function $v_R \in L^2(\Omega)$ as:
\begin{equation}
    v_R(x) = 
    \begin{cases} 
        \frac{1}{\sqrt{2\mu_R}} & \text{if } x \in B_1, \\
        -\frac{1}{\sqrt{2\mu_R}} & \text{if } x \in B_2, \\
        0 & \text{otherwise}.
    \end{cases}
\end{equation}
By construction, $\int_\Omega v_R(x) \, dx = 0$ and $\|v_R\|_{L^2}^2 = 1$. Thus, $v_R \in A_1$.

We evaluate the quadratic form $\langle \mathcal{L}v_R, v_R \rangle$ by expanding the local multiplication part and the non-local integral part
\begin{equation}
    \langle \mathcal{L}v_R, v_R \rangle = \iint_{\Omega \times \Omega} J(x-y)v_R(x)v_R(y) \, dx dy - \int_\Omega b(x) v_R(x)^2 \, dx.
\end{equation}
Using the definition of $v_R$, the local part simplifies to:
\begin{equation}
    - \int_\Omega b(x) v_R(x)^2 \, dx = - \frac{1}{2\mu_R} \int_{B_1} b(x) \, dx - \frac{1}{2\mu_R} \int_{B_2} b(x) \, dx.
\end{equation}
For the integral part, the product $v_R(x)v_R(y)$ is non-zero only on the sub-domains $B_1 \times B_1$, $B_2 \times B_2$, and the cross-terms $B_1 \times B_2$ and $B_2 \times B_1$. Taking symmetry into account, the integral expands to
\begin{align}
    \begin{split}
        \iint_{\Omega \times \Omega} J(x-y)v_R(x)v_R(y) \, dx dy 
        &=
        \frac{1}{2\mu_R} \iint_{B_1 \times B_1} J(x-y) \, dx dy \\
        &+ \frac{1}{2\mu_R} \iint_{B_2 \times B_2} J(x-y) \, dx dy \\
        &- \frac{1}{\mu_R} \iint_{B_1 \times B_2} J(x-y) \, dx dy.
    \end{split}
\end{align}
Define the difference $\Delta(R) := \langle \mathcal{L}v_R, v_R \rangle - \sup \sigma_c$. Since the following identity holds true
\begin{equation}
    \sup \sigma_c = -b(x_1) = \frac{1}{2\mu_R} \int_{B_1} (-b(x_1)) \, dx + \frac{1}{2\mu_R} \int_{B_2} (-b(x_1)) \, dx
\end{equation}
we re-write $\Delta(R)$ to obtain equation
\begin{align}
    \begin{split}
        \label{eq:delta_R_exact}
        \Delta(R) 
        &= \frac{1}{2\mu_R} \int_{B_1} \big( b(x_1) - b(x) \big) \, dx + \frac{1}{2\mu_R} \int_{B_2} \big( b(x_1) - b(x) \big) \, dx \nonumber \\
        &\quad + \frac{1}{2\mu_R} \iint_{B_1 \times B_1} J(x-y) \, dx dy + \frac{1}{2\mu_R} \iint_{B_2 \times B_2} J(x-y) \, dx dy \\
        &- \frac{1}{\mu_R} \iint_{B_1 \times B_2} J(x-y) \, dx dy.
    \end{split}
\end{align}
To evaluate the asymptotic behavior of $\Delta(R)$ as $R \to 0$ (while holding the centers $x_1, x_2$ fixed), we apply the Lebesgue Differentiation Theorem to the averages over the shrinking balls $B_1$ and $B_2$:
\begin{align} 
    \begin{split}
        \label{eq:limit_b}
        \lim_{R \to 0} \frac{1}{2\mu_R} \int_{B_1} \big( b(x_1) - b(x) \big) \, dx &= 0, \\
        \lim_{R \to 0} \frac{1}{2\mu_R} \int_{B_2} \big( b(x_1) - b(x) \big) \, dx &= \frac{1}{2} \big( b(x_1) - b(x_2) \big).
    \end{split}
\end{align}
For the double integrals representing the integral operator, the variables $x$ and $y$ become localized near the centers of their respective domains as $R \to 0$. Hence, the diagonal terms converge to the kernel evaluated at zero:
\begin{align} 
    \begin{split}
        \label{eq:limit_J_diag}
        \lim_{R \to 0} \frac{1}{\mu_R^2} \iint_{B_1 \times B_1} J(x-y) \, dx dy &= J(0), \\
        \lim_{R \to 0} \frac{1}{\mu_R^2} \iint_{B_2 \times B_2} J(x-y) \, dx dy &= J(0).
    \end{split}
\end{align}
Similarly, the cross-term converges to the kernel evaluated at the fixed distance $D = |x_1 - x_2|$:
\begin{equation} 
    \label{eq:limit_J_cross}
    \lim_{R \to 0} \frac{1}{\mu_R^2} \iint_{B_1 \times B_2} J(x-y) \, dx dy = J(|x_1 - x_2|) = J(D).
\end{equation}

Dividing equation \eqref{eq:delta_R_exact} by $\mu_R$ and taking the limit $R \to 0$, we substitute the point evaluations from \eqref{eq:limit_b}, \eqref{eq:limit_J_diag}, and \eqref{eq:limit_J_cross}:
\begin{align} 
    \label{eq:delta_limit}
    \lim_{R \to 0} \frac{\Delta(R)}{\mu_R} 
    &= 0 + \frac{1}{2} \big( b(x_1) - b(x_2) \big) + \frac{1}{2} J(0) + \frac{1}{2} J(0) - J(D) \nonumber \\
    &= J(0) - J(D) - \frac{1}{2} \big( b(x_2) - b(x_1) \big).
\end{align}

From the inequality \eqref{eq:strict_gap_condition}, we know that $b(x_2) - b(x_1) < J(0) - J(D)$. It follows that
\begin{equation}
    J(0) - J(D) - \frac{1}{2} \big( b(x_2) - b(x_1) \big) > \frac{1}{2} \big( J(0) - J(D) \big) > 0.
\end{equation}

Since $\lim_{R \to 0} \frac{\Delta(R)}{\mu_R} > 0$ and $\mu_R > 0$ for any $R > 0$, there must exist a sufficiently small radius $R_0 > 0$ such that $\Delta(R_0) > 0$. 
This implies that for the specific test function $v_{R_0} \in A_1$, the energy satisfies:
\begin{equation}
    \langle \mathcal{L}v_{R_0}, v_{R_0} \rangle > \sup \sigma_c.
\end{equation}
Therefore, we have that
\begin{equation}
    \beta_1 = \sup_{v \in A_1} \langle \mathcal{L}v, v \rangle \geq \langle \mathcal{L}v_{R_0}, v_{R_0} \rangle > \sup \sigma_c,
\end{equation}
