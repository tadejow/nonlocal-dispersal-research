...
\end{equation}
where $\Omega \subset \mathbb{R}^n$. We assume the following properties of the integral kernel $J$.
...
\begin{lem}
    \label{lem:strict_separation}
    Let $\beta_k$ be an eigenvalue of $\mathcal{L}$ satisfying $\beta_k > \sup \sigma_c$. Let $E_k$ denote the eigenspace associated with $\beta_k$. Define the set $A_{k+1} = \{v \in A_1 : v \perp \bigoplus_{i=1}^k E_i, \|v\|_{L^2} = 1\}$. 
    Then, there exists $\delta > 0$ such that
    \begin{equation}
        \beta_{k+1} := \sup_{v \in A_{k+1}} \langle \mathcal{L}v, v \rangle \leq \beta_k - \delta.
    \end{equation}
\end{lem}

\begin{proof}
We consider two cases. If $\beta_{k+1} = \sup \sigma_c$, then the results follows trivially. Otherwise, since $\mathcal{L}$ is a bounded, self-adjoint operator on the Hilbert space $L^2(\Omega)$, its spectrum $\sigma(\mathcal{L})$ is a closed subset of $\mathbb{R}$. By Weyl's theorem, any element of the spectrum located strictly above the essential (continuous) spectrum $\sigma_c$ must be an isolated eigenvalue of finite multiplicity. This proves the Lemma.
\end{proof}
